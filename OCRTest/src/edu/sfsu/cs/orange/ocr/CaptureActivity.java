...
public final class CaptureActivity extends Activity implements SurfaceHolder.Callback, 
  ...
  public void addExpense(View v){
		//Toast.makeText(getApplicationContext(), "ADD EXPENSE!", Toast.LENGTH_LONG).show();
		TextView tv = (TextView)findViewById(R.id.ocr_result_text_view);
		Spinner mySpinner=(Spinner) findViewById(R.id.spinnerCategory1);
		...
		try{ //  123.00 
			...
			Toast.makeText(getApplicationContext(), "This is not a number! Please try again.\nTry focusing on just the number!", 
					   Toast.LENGTH_LONG).show();
			return;
		}
		Toast.makeText(getApplicationContext(), "Adding "+integer+" to your '"+text.toLowerCase()+"' budget. Please wait.", 
				   Toast.LENGTH_LONG).show();
		...
		b.expense = integer;
		b.category = text;
		b.date = (new DateToday()).getTodayDate();
		String toastThis = db.addExpense(b);
		Toast.makeText(getApplicationContext(), toastThis, 
				   Toast.LENGTH_SHORT).show();
  ...
  public void onCreate(Bundle icicle) {
    ...
    final Toast tag = Toast.makeText(getApplicationContext(),"Scan a bill to add it to your budget!\nFocus on the number for best results!",Toast.LENGTH_LONG);
	tag.show();
  ...
  private void setDefaultPreferences() {
	Toast.makeText(getApplicationContext(), "Please add a budget to start using this app.", Toast.LENGTH_LONG).show();
    prefs = PreferenceManager.getDefaultSharedPreferences(this);
